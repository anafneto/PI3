...
const Propostas = ({ dadosPropostas, onVerMais }) => {
  ...
  /**
   * Usamos useMemo para recomputar a lista filtrada somente quando:
   * - dadosPropostas mudar, ou
   * - qualquer um dos filtros mudar.
   */
  const propostasFiltradas = useMemo(() => {
    return dadosPropostas.filter((p) => {
      // Se filtroLocalizacao não estiver vazio, checamos se bate
      const bateLocalizacao =
        filtroLocalizacao === "" || p.localizacao === filtroLocalizacao;

      // Se filtroTipoContrato não estiver vazio, checamos se bate
      const bateContrato =
        filtroTipoContrato === "" || p.tipoContrato === filtroTipoContrato;

      // Se filtroHorario não estiver vazio, checamos se bate
      const bateHorario = filtroHorario === "" || p.horario === filtroHorario;

      // Só retorna true se bater em todos
  ...
  return (
    <div className="mt-3">
      ...
      <div className="row g-2 align-items-center mb-4">
        ...
        <div className="col-sm-6 col-md-12 col-lg-3 d-flex justify-content-lg-end">
          <button
            type="button"
              ...
              <div className={colClasses} key={idx}>
                <div className="card h-100 shadow-sm">
                  <div className="card-body p-2">
                    ...
                    {/* Exibimos localização / tipo de contrato / horário abaixo do título, se quiser */}
                    <p
                      className="card-text text-secondary mb-1"
                      style={{ fontSize: "0.75rem" }}
                    >
                      {proposta.localizacao} &bull; {proposta.tipoContrato} &bull; {proposta.horario}
                    </p>
                    <p
      ...
      <div className="d-flex justify-content-end mt-4">
        <button className="btn btn-dark" onClick={onVerMais} style={{ minWidth: "100px" }}>
          Ver mais
        </button>
      </div>
    </div>
